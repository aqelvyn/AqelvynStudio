...
// POST { key, wallet, chain }  (Authorization: Bearer <session>)
// Returns the master build-prompt ONLY if the authenticated wallet has unlocked
// this key. Content never ships to the client otherwise.
export async function POST(req: Request) {
  ...
  const unlock = await getUnlock(address, key);
  if (!unlock) return NextResponse.json({ error: 'locked' }, { status: 403 });
